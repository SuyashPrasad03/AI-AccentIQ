...
**Backend → Deepgram**: CPU inference for ASR + forced alignment. Background job (FastAPI BackgroundTasks). Model: `small` on CPU.
...
|---|---|---|
| **Deepgram** over plain Whisper | Forced alignment gives word-level timestamps — the feature that enables per-word mistake highlighting | Plain Whisper only gives segment timestamps |
...
| **FastAPI** over Django/Express | Async-first (matches Motor + Deepgram pipeline), native Pydantic validation, auto-generated docs | Django (sync overhead), Express (weaker validation) |
...
- **Confidence**: ASR confidence (0-1) from Deepgram per word
...
| **CPU Deepgram** vs. GPU | ~30s per 30s clip on CPU. Fine for demo. | GPU-backed Render instance or RunPod |
...
2. **GPU-backed Deepgram** for <5s processing latency
